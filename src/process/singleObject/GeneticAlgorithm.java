...
//        return individual.vehicleList.size();
    ...
    public void reconstructChromosome(Individual individual,List<Order> orderList, SetDataMap setDataMap){
        int index=0;
        for (Vehicle v:individual.vehicleList) {
            for (Integer gene:v.chromosome) {
                individual.setGene(index++,gene);
            }
        }
//        int[] reconstructed=getReconstructedChromosome(individual);
//        List<Vehicle> tmpVehicleList=new ArrayList<>();
//        tmpVehicleList=new Vehicle().run(reconstructed,orderList,setDataMap);
...
//        }
    ...
    public int[] getReconstructedChromosome(Individual individual){
        int index=0;
        int[] reconstructedChromosome=new int[individual.getChromosomeLength()];
        for (Vehicle v:individual.vehicleList) {
            for (Integer gene:v.chromosome) {
                reconstructedChromosome[index++]=gene;
            }
        }
        return reconstructedChromosome;
    }
...
//            long calcFitnessTime=System.currentTimeMillis();
//            System.out.println(calcFitnessTime-startTime);
